...
class MultiHeadAttention:
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        :param key_padding_mask: (Bsz, Sk)
        :param attn_mask: (Sq, Sk)
        :return: (Bsz, H, Sq, Sk)
        """
        # TODO: Implement merge masks

        # Expand key_padding_mask to (N, 1, 1, S) and broadcast to (N, H, L, S)
        key_mask = np.expand_dims(key_padding_mask, axis=(1, 2))  # (N, 1, 1, S)
        ...
        attention_mask = np.expand_dims(attn_mask, axis=(0, 1))  # (1, 1, L, S)
