...
class DecoderOnlyTransformer(nn.Module):
    ...
    def forward(self, padded_targets: torch.Tensor, target_lengths: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, dict]:
        ...
        if target_lengths is not None:
            pad_mask_dec = PadMask(padded_targets, target_lengths)  # bsz, T; True/False
        
        # TODO: Create causal mask to prevent attending to future tokens on the same device as the input (use CausalMask)
        # (T, T)
        causal_mask = CausalMask(padded_targets)  # T, Ture/False
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.enc_layers = nn.ModuleList([
            SelfAttentionEncoderLayer(d_model, num_encoder_heads, d_ff_encoder, layer_drop_rate)
        ...
        self.dec_layers = nn.ModuleList([
            CrossAttentionDecoderLayer(d_model, num_decoder_heads, d_ff_decoder, layer_drop_rate)
    ...
    def encode(
            ...
            source_lengths: torch.Tensor,  # for pad_mask_src
        ...
        ctc_inputs = {
            "log_probs": ctc_logits.transpose(0, 1),  # (B, T, C) -> (T, B, C)
    ...
    def decode(
        self, 
        padded_targets: torch.Tensor,  # also for causal_mask
        encoder_output: torch.Tensor,
        target_lengths: Optional[torch.Tensor] = None,  # for pad_mask_tgt
        pad_mask_src: Optional[torch.Tensor] = None,  # pad_mask_src
        ...
        for i in range(self.num_decoder_layers):
            if self.training and self.layer_drop_rate > 0 and random.random() < self.layer_drop_rate:
                continue
            # TODO: Pass through decoder layer
            x_dec, self_attn, cross_attn = self.dec_layers[i](x_dec, encoder_output,
