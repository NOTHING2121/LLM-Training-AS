...
class SequenceGenerator:
    ...
    def generate_greedy(
        ...
        scores = torch.zeros(batch_size, device=self.device)  # 每个batch的分数
        finished = torch.zeros(batch_size, dtype=torch.bool, device=self.device)  # 每个batch的结束与否

        for t in range(0, self.max_length - sequence_length):
            # if all sequences are finished
            if finished.all():
                break

            # logits
            next_scores = self.score_fn(x)  # x: (batch_size, sequence_length_t), next_scores: (batch_size, vocab_size)
            logits = self._apply_repeat_penalty(next_scores, x, repeat_penalty)
            ...
            # token_scores = log_probs[batch_idx, next_tokens]
            token_scores = log_probs.gather(1, next_tokens.unsqueeze(1)).squeeze(1)  # (batch_size,)  # 对每个batch，取出log_probs中对应next_token的log概率

            # update scores for unfinished
            # is_unfinished = (~finished)
            # scores = scores + token_scores * is_unfinished
            scores = torch.where(finished, scores, scores + token_scores)  # (batch_size,)  # 没结束的样本：继续累加当前token的log概率；已经结束的样本：分数保持不变

            # concat
            x = torch.cat([x, next_tokens.unsqueeze(1)], dim=1)  # (batch_size, sequence_length_t + 1)

            # update finished if eos
            is_eos = (next_tokens == self.tokenizer.eos_id)
            finished = finished | is_eos  # 结束 | eos -> 结束， 结束 | 非eos -> 结束， 未结束 | eos -> 结束
    ...
    def generate_beam(
        ...
        batch_size, sequence_length = x.size()
        scores = torch.zeros(batch_size, beam_width, device=self.device)  # 每个batch的分数
        finished = torch.zeros(batch_size, beam_width, dtype=torch.bool, device=self.device)  # 每个batch的结束与否

        # ✅ 第一步：从单一输入扩展出 beam_width 条不同路径
        ...
        # 生成初始 beam
        x_beam = torch.cat([x.unsqueeze(1).repeat(1, beam_width, 1), top_tokens.unsqueeze(-1)], dim=-1)
        scores = top_scores
        finished = (top_tokens == self.tokenizer.eos_id)

        # # init
        # x_beam = x.unsqueeze(1).repeat(1, beam_width, 1)  # (batch_size, beam_width, sequence_length)

        for t in range(0, self.max_length - sequence_length - 1):
            # if all sequences are finished
            if finished.all():
                break
            ...
            for k in range(beam_width):
                next_score = self.score_fn(x_beam[:, k, :])
                next_scores.append(next_score.unsqueeze(1))
            next_scores = torch.cat(next_scores, dim=1)  # next_scores: (batch_size, beam_width, vocab_size)

            logits = self._apply_repeat_penalty(next_scores, x_beam, repeat_penalty)  # (batch_size, beam_width, vocab_size)

            # temperature
            logits = logits / temperature
            log_probs = nn.functional.log_softmax(logits, dim=-1)

            # 在 flatten 前，对 token 维度添加 tie-breaking 偏移
            vocab_size = log_probs.size(-1)
            token_offset = (
                    torch.arange(vocab_size, device=self.device).float() * 1e-8
            ).view(1, 1, vocab_size)

            flat_total_scores = (scores.unsqueeze(-1) + log_probs + token_offset).view(batch_size, -1)

            top_scores, top_indices = torch.topk(flat_total_scores, beam_width, dim=-1)  # (B, beam)

            vocab_size = log_probs.size(-1)
            beam_indices = top_indices // vocab_size
            token_indices = top_indices % vocab_size

            new_x_beam = torch.zeros(batch_size, beam_width, x_beam.shape[-1] + 1, dtype=torch.long, device=self.device)
            new_finished = torch.zeros_like(finished)

            # for b in range(batch_size):
            #     for beam in range(beam_width):
            #         prev_beam = beam_indices[b, beam]
            #         next_token = token_indices[b, beam]
            #
            #         new_x_beam[b, beam] = torch.cat([x_beam[b, prev_beam], next_token.view(1)], dim=0)
            #
            #         is_eos = (next_token == self.tokenizer.eos_id)
            #         new_finished[b, beam] = finished[b, prev_beam] | is_eos

            # gather old beams
            batch_idx = torch.arange(batch_size, device=self.device).unsqueeze(1).expand(-1, beam_width)
            selected_beams = x_beam[batch_idx, beam_indices]  # (B, beam, seq_len)

            # append new tokens
            new_x_beam = torch.cat([selected_beams, token_indices.unsqueeze(-1)], dim=-1)  # (B, beam, seq_len+1)

            # update finished mask
            is_eos = (token_indices == self.tokenizer.eos_id)
            new_finished = finished.gather(1, beam_indices) | is_eos
        ...
        sorted_scores, sort_indices = torch.sort(scores, descending=True, dim=-1)
        sorted_x = torch.gather(x_beam, dim=1, index=sort_indices.unsqueeze(-1).expand(-1, -1, x_beam.shape[-1]))
