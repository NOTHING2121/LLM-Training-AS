    ...
    def _train_epoch(self, dataloader) -> Tuple[Dict[str, float], Dict[str, torch.Tensor]]:
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                raw_preds, attn_weights = self.model(targets_shifted, lengths)  # (B, T, V)
                ...
                raw_loss = self.criterion(
                    raw_preds.reshape(-1, raw_preds.shape[-1]),  # (B*T, V)
                    targets_golden.reshape(-1)  # (B*T)
                )
    ...
    def generate(self, dataloader, generation_config: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        for _, (prompt, seq, score, original) in enumerate(zip(prompts, processed_seqs, scores, originals)):
            results.append({
                ...
                'original': self.tokenizer.decode(original[len(prompt):].tolist()),  # original[len(prompt) ~ T]
                'generated': self.tokenizer.decode(seq[len(prompt):].tolist()),  # seq[len(prompt) ~ T]
                'score': score.item()
            })
