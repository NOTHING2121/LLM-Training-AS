...
class ASRTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        self.ctc_criterion = self.config["loss"].get("ctc_weight", 0.0)
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                seq_out, curr_att, ctc_inputs = self.model(
                    ...
                    feat_lengths, transcript_lengths
                )
                ...
                ce_loss = self.ce_criterion(
                    seq_out.reshape(-1, seq_out.shape[-1]),  # (B*T, V)
                    targets_golden.reshape(-1),  # (B*T)
                )

                # TODO: Calculate CTC loss if needed
                if self.ctc_weight > 0:
                    log_probs = ctc_inputs["log_probs"] # (T, B, D)
                    src_lengths = ctc_inputs["lengths"] # (B,)
                    ctc_loss = self.ctc_criterion(
                        log_probs, targets_golden,
                        src_lengths, transcript_lengths
                    )
